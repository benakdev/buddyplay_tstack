...
import { decrementUnreadCount, incrementUnreadCount } from './lib/notifications';
import {
  getAvailabilityOverlapPercent,
  getMatchPercentage,
  getRatingClosenessPercent,
  haveCompatibleGenderPreferences,
...
import { sportProfileSchema, sportProfileUpdateArgsSchema, sportProfileUpsertSchema } from './lib/zodSchemas';
...
const pickleballSkillSchema = z.number().min(SKILL_RANGES.Pickleball.min).max(SKILL_RANGES.Pickleball.max);
...
function getPlaytomicMultiplierForGender(gender?: string): number {
  return gender === 'Female' ? WOMEN_PLAYTOMIC_MULTIPLIER : MEN_PLAYTOMIC_MULTIPLIER;
}

function requireProfileGender(gender?: string): void {
  if (!gender) {
    throw new Error('Please complete your profile (set your gender) before creating a Sport Passport.');
  }
}
 ...
 */
function validatePadelRatings(playtomicRating?: number, wprRating?: number): void {
...
function deriveSkillLevel(
  ...
  const skillLevelForParse = sport === 'Pickleball' ? (args.skillLevel ?? fallbackSkillLevel) : args.skillLevel;
  ...
  const derived =
    sport === 'Padel'
      ? (args.playtomicRating ??
        (args.wprRating !== undefined ? derivePlaytomicFromWpr(args.wprRating, gender) : undefined))
...
export const upsertSportProfile = zMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingProfile = await ctx.db
      ...
      .withIndex('by_user_sport', q => q.eq('userId', userId).eq('sport', args.sport))
      .unique();
    ...
    await ctx.scheduler.runAfter(0, internal.sportProfiles.notifyMatchingUsers, { profileId });
    return profileId;
  }
});
...
export const createSportProfile = zMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingProfile) {
      throw new Error(`You already have a profile for ${args.sport}. Use update instead.`);
    }
...
export const updateSportProfile = zMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (profile.sport === 'Pickleball' && args.skillLevel !== undefined) updates.skillLevel = args.skillLevel;
    if (profile.sport === 'Padel' && (args.playtomicRating !== undefined || args.wprRating !== undefined)) {
      updates.skillLevel = derivedSkillLevel;
    }
    if (args.homeClubId !== undefined) updates.homeClubId = args.homeClubId;
    if (profile.sport === 'Padel' && args.playtomicRating !== undefined) updates.playtomicRating = args.playtomicRating;
    if (profile.sport === 'Padel' && args.wprRating !== undefined) updates.wprRating = args.wprRating;
    if (args.matchingTolerance !== undefined) updates.matchingTolerance = args.matchingTolerance;
    if (args.attributes !== undefined) updates.attributes = args.attributes;
    if (args.preferredGender !== undefined) updates.preferredGender = args.preferredGender;
    ...
    await ctx.scheduler.runAfter(0, internal.sportProfiles.notifyMatchingUsers, { profileId: args.profileId });
    return null;
  }
});
...
export const deleteSportProfile = zMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const notificationsByMatch = await ctx.db
      .query('notifications')
      .withIndex('by_matching_profile', q => q.eq('matchingProfileId', args.profileId))
      .collect();

    const notificationsToRemove = new Map<string, (typeof notificationsByProfile)[number]>();
...
export const findMatchingPlayers = zQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const currentProfile = await ctx.db.get('userSportProfiles', args.profileId);
    if (!currentProfile) {
      throw new Error('Profile not found');
    }
    ...
    const profiles = await ctx.db
      .query('userSportProfiles')
      .withIndex('by_sport_active_level', q => q.eq('sport', currentProfile.sport).eq('isActive', true))
      .collect();
    ...
    for (const profile of profiles) {
      ...
      if (
        !haveCompatibleGenderPreferences({
          currentPreferredGender: currentProfile.preferredGender,
          currentGender: currentUserGender,
          candidatePreferredGender: profile.preferredGender,
          candidateGender
        })
      ) {
        continue;
      ...
      if (
        currentProfile.sport === 'Padel' &&
        !haveCompatiblePadelTiers({
          currentWprRating: currentProfile.wprRating,
          currentGender: currentUserGender,
          candidateWprRating: profile.wprRating,
          candidateGender
        })
      ) {
        continue;
      ...
      if (
        !isReciprocalSkillMatch({
          currentSkillLevel: currentProfile.skillLevel,
          currentTolerance: args.tolerance ?? currentProfile.matchingTolerance,
          candidateSkillLevel: profile.skillLevel,
          candidateTolerance: profile.matchingTolerance
        })
      ) {
        continue;
      }

      const sameClub = isSameClub(currentProfile.homeClubId, profile.homeClubId);
      const ratingClosenessPercent = getRatingClosenessPercent(currentProfile.skillLevel, profile.skillLevel);
      const availabilityOverlapPercent = getAvailabilityOverlapPercent(
    ...
    results.sort((a, b) => {
      ...
      const aPrimary = a.sameClub ? (a.matchPercentage ?? 0) : a.ratingClosenessPercent;
      const bPrimary = b.sameClub ? (b.matchPercentage ?? 0) : b.ratingClosenessPercent;
    ...
    return results.slice(0, limit).map(({ profile, user, sameClub, matchPercentage }) => ({
      profile,
      user,
      sameClub,
      matchPercentage
...
export const notifyMatchingUsers = zInternalMutation({
  ...
  handler: async (ctx, { profileId }) => {
    ...
    const existingByKey = new Map<string, (typeof existingMatchNotifications)[number]>();
    ...
    // 2. Initial Checks
    if (!profile.isActive || !profile.homeClubId || profile.skillLevel === undefined) {
    ...
    const candidates = await ctx.db
      .query('userSportProfiles')
      .withIndex('by_sport_active_level', q => q.eq('sport', profile.sport).eq('isActive', true))
      .collect();

    console.log(`[PlayerMatch] Found ${candidates.length} active candidates for ${profile.sport}`);

    const userIds = Array.from(new Set([profile.userId, ...candidates.map(candidate => candidate.userId)]));
    const users = await Promise.all(userIds.map(userId => ctx.db.get('users', userId)));
    const userById = new Map(users.filter(Boolean).map(userDoc => [userDoc!._id, userDoc!]));

    const clubIds = Array.from(
      new Set(
        [profile.homeClubId, ...candidates.map(candidate => candidate.homeClubId)].filter(
          (clubId): clubId is Id<'clubs'> => clubId !== undefined
        )
      )
    );
    const clubs = await Promise.all(clubIds.map(clubId => ctx.db.get('clubs', clubId)));
    const clubById = new Map(clubs.filter(Boolean).map(club => [club!._id, club!]));
    ...
    for (const candidate of candidates) {
      ...
      if (!isSameClub(profile.homeClubId, candidate.homeClubId)) {
        console.log(
      ...
      const matches = isReciprocalSkillMatch({
        currentSkillLevel: pLevel,
        currentTolerance: profile.matchingTolerance,
        candidateSkillLevel: cLevel,
        candidateTolerance: candidate.matchingTolerance
      });

      console.log(
        `[PlayerMatch] Checking candidate ${candidate.userId}: Levels P:${pLevel} vs C:${cLevel}. Reciprocal match? ${matches}`
      );
      ...
      if (matches) {
        ...
        const profileHandle = profileUser?.username ? `@${profileUser.username}` : 'A player';
        const candidateHandle = candidateUser?.username ? `@${candidateUser.username}` : 'A player';
        const profileClubName = profile.homeClubId ? clubById.get(profile.homeClubId)?.name : undefined;
        const candidateClubName = candidate.homeClubId ? clubById.get(candidate.homeClubId)?.name : undefined;
        const profileClubText = profileClubName ? `at ${profileClubName}` : 'at your club';
        const candidateClubText = candidateClubName ? `at ${candidateClubName}` : 'at your club';
