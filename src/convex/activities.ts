...
export const listMyUpcomingActivities = zQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // Get JOINED and PENDING participations for this user
    const participations = await ctx.db
      .query('activityParticipants')
      .withIndex('by_user', q => q.eq('userId', user._id))
      .filter(q => q.or(q.eq(q.field('status'), 'JOINED'), q.eq(q.field('status'), 'PENDING')))
      .collect();
    ...
    // Build a map of activityId → participation info
    const participationMap = new Map(participations.map(p => [p.activityId, p]));
    ...
    // Filter out nulls and activities that user hosts or are not upcoming
    const upcomingActivities = fetchedActivities
      .filter(
        (a): a is (typeof fetchedActivities)[0] & {} =>
          a != null && a.creatorId !== user._id && a.startTime >= afterTime
      )
    ...
    // Look up requestId for PENDING participations
    const withStatus = await Promise.all(
      enriched.map(async item => {
        const p = participationMap.get(item.activity._id);
        const status = (p?.status ?? 'JOINED') as 'JOINED' | 'PENDING';

        let requestId = null;
        if (status === 'PENDING') {
          const req = await ctx.db
            .query('requests')
            .withIndex('by_activity_user', q => q.eq('activityId', item.activity._id).eq('userId', user._id))
            .filter(q => q.eq(q.field('status'), 'PENDING'))
            .unique();
          requestId = req?._id ?? null;
        }

        return { ...item, participantStatus: status, requestId };
      })
    );
