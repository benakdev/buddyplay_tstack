...
export function ChatToolbar({ children, className, ...props }: ChatToolbarProps) {
  return (
    <div
      ...
      {...props}
    >
      <div className={cn('flex items-end gap-2')}>{children}</div>
    </div>
  );
}
 ...
 * ```
 */
export function ChatToolbarTextarea({ className, onSubmit, onKeyDown, ...props }: ChatToolbarTextareaProps) {
 ...
 * ```
 */
export function ChatToolbarAddon({ children, className, align = 'inline-start', ...props }: ChatToolbarAddonProps) {
  return (
    <div className={cn('flex h-10 items-center gap-1.5', chatToolbarAddonAlignStyles[align], className)} {...props}>
      {children}
    </div>
  );
}
