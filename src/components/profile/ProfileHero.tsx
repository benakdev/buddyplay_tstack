...
export function ProfileHero({
	...
	// Derive display name from Clerk or profileData
	const displayName = React.useMemo(() => {
		if (profileData) return profileData.name;
		if (!clerkUser) return "";
		...
		if (user?.privacySettings?.hideLastName) {
			...
			return `${firstName}${lastInitial}`.trim() || "User";
		}

		return `${firstName} ${lastName}`.trim() || "User";
	}, [clerkUser, user, profileData]);
	...
	const finalImageUrl = profileData?.imageUrl || clerkUser?.imageUrl;

	return (
		<div
			className={cn(
				"flex flex-col items-center gap-8 text-center sm:flex-row sm:items-start sm:text-left",
				className,
			)}
		>
			{/* Avatar */}
			...
			<div className="min-w-0 flex-1 pt-2">
				...
				<div className="flex items-center justify-center gap-3 sm:justify-start">
					...
					{!isReadOnly && (
						<ProfileSettingsModal
							...
							onOpenChange={onProfileSettingsOpenChange}
						/>
					)}
				</div>
				...
				<div className="text-muted-foreground mt-2 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 sm:justify-start">
					{finalUsername && (
						<span className="font-medium">@{finalUsername}</span>
					)}

					{finalUsername && finalLocation?.city && (
				...
				<div className="mt-6">
					{finalBio ? (
						...
						<p className="text-muted-foreground/60 italic">
							{isReadOnly
								? "No bio provided."
								: "Add a bio in settings to tell others about yourself."}
						</p>
					)}
				</div>
			</div>
