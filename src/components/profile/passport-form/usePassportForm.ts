...
import { type PassportFormValues, createPassportFormSchema } from '@/lib/schema/passportForm';
...
export function usePassportForm({ sport, mode, profileId, initialData, onSuccess }: UsePassportFormOptions) {
  ...
  const formSchema = React.useMemo(() => createPassportFormSchema(sport), [sport]);

  const form = useForm({
    defaultValues: {
      skillLevel: initialData?.skillLevel ?? (sport === 'Pickleball' ? skillRange.min : undefined),
      ...
      playtomicRating: initialData?.playtomicRating ?? (sport === 'Padel' ? skillRange.min : undefined),
    ...
    onSubmit: async ({ value }) => {
      ...
      if (sport === 'Padel' && value.wprRating !== undefined && !currentUser?.gender) {
      ...
      try {
        const sharedPayload = {
          ...
          homeClubId,
          playtomicRating: sport === 'Padel' ? value.playtomicRating : undefined,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error('Failed to delete passport, please try again or contact support.');
      console.error(error);
    } finally {
      setIsSubmitting(false);
    }
