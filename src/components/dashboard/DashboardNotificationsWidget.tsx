...
export function DashboardNotificationsWidget() {
  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-col items-start justify-between gap-2 px-0.5 sm:flex-row sm:items-center">
      ...
      {notifications === undefined ? (
        <div className="flex gap-3 overflow-hidden">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-muted/40 h-44 w-[min(300px,86vw)] shrink-0 animate-pulse rounded-2xl border" />
          ))}
        </div>
      ) : notifications.length === 0 ? (
        <div className="bg-muted/20 flex items-center gap-3 rounded-2xl border px-5 py-5">
        ...
        <Carousel opts={{ align: 'start', dragFree: true }} className="w-full">
          <CarouselContent className="-ml-3">
            {notifications.map(item => (
              <CarouselItem key={item.notification._id} className="basis-[min(300px,86vw)] pl-3">
                <NotifCard item={item} />
              </CarouselItem>
            ))}
          </CarouselContent>
